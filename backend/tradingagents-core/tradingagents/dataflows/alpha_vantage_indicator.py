...
def _build_indicator_params(
    ...
    function_name, _target_column, mapped_period, required_series_type = SUPPORTED_INDICATORS[indicator]
    params = {
        "symbol": symbol,
        "interval": interval,
        "datatype": "csv",
...
def _parse_indicator_csv(
    ...
    if "time" not in reader.fieldnames:
        return [], f"Error: 'time' column not found in data for {indicator}. Available columns: {reader.fieldnames}"
...
def _format_indicator_result(
    ...
    if rows:
        values = "".join(f"{date_value.strftime('%Y-%m-%d')}: {value}\n" for date_value, value in rows)
    else:
