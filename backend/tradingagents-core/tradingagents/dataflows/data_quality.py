...
def validate_ohlcv(result) -> dict:
    if isinstance(result, str):
        if looks_missing(result):
            return _quality(available=False, is_empty=True, missing_fields=["rows"], warnings=[result.splitlines()[0] if result else "empty OHLCV"])
        dates = extract_price_dates(result)
        warnings = [] if len(dates) >= 10 else [f"Only {len(dates)} OHLCV rows detected."]
        return _quality(available=bool(dates), confidence="high" if len(dates) >= 30 else "medium", is_empty=not bool(dates), warnings=warnings)
    ...
    return _quality(available=not missing, confidence="high" if len(rows) >= 30 else "medium", is_empty=not rows, missing_fields=missing, warnings=warnings)


def validate_fundamentals(result) -> dict:
    if isinstance(result, str):
        if looks_missing(result):
            return _quality(available=False, is_empty=True, missing_fields=["fundamentals"], warnings=[result.splitlines()[0] if result else "empty fundamentals"])
    ...
    return _quality(available=bool(metrics or company), confidence="medium" if missing or warnings else "high", is_empty=not bool(metrics or company), missing_fields=missing, warnings=warnings)


def validate_news(result) -> dict:
    if isinstance(result, str):
        if looks_missing(result):
            return _quality(available=False, is_empty=True, missing_fields=["news"], warnings=[result.splitlines()[0] if result else "empty news"])
        article_count = result.count("### ")
        return _quality(available=True, confidence="high" if article_count >= 3 else "medium", warnings=[] if article_count else ["No markdown article headings detected."])
    if not isinstance(result, dict):
        return _quality(available=False, is_empty=True, missing_fields=["items"])
    items = result.get("items") if isinstance(result.get("items"), list) else []
    return _quality(available=bool(items), confidence="high" if len(items) >= 3 else "medium", is_empty=not bool(items), missing_fields=[] if items else ["items"])


def validate_sentiment(result) -> dict:
    if isinstance(result, str):
        if looks_missing(result) or "\"available\": false" in result.lower():
            return _quality(available=False, is_empty=True, missing_fields=["sentiment"], warnings=["Sentiment unavailable; do not fabricate it from unrelated fields."])
    ...
    if result.get("available") is False:
        return _quality(available=False, is_empty=True, missing_fields=["sentiment"], warnings=[str(result.get("reason") or "Sentiment unavailable")])
    return _quality(available=True, confidence="medium")
