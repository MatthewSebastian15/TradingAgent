...
For Buy and Sell recommendations, trade levels must use Risk:Reward between 1:3 and 1:5 only. In numeric fields, risk_reward_ratio means reward divided by risk, so valid values are 3.0 to 5.0. For Buy, stop_loss must be below entry_price and take_profit above entry_price. For Sell, stop_loss must be above entry_price and take_profit below entry_price. Use price_target as an analyst/fair target and take_profit as the trade execution target. Do not invent current_price. Allowed volatility_level values are only Low, Medium, High, or Very High.
{_horizon_instruction(time_horizon_text)}
...
Use LAST CLOSE PRICE as the current market price anchor for entry_price, stop_loss, take_profit, and price_target. If LAST CLOSE PRICE is unavailable or data quality is not usable, leave unsupported price fields null instead of inventing numbers. Do not invent current_price.
For Buy and Sell, price_target, entry_price, stop_loss, take_profit, max_drawdown_estimate, volatility_level, rebalancing_action, and risk_reward_ratio must be non-null when the data supports an actionable decision. For Buy, stop_loss must be below entry_price and take_profit above entry_price. For Sell, stop_loss must be above entry_price and take_profit below entry_price. For Hold, include only current_price, volatility_level, and rebalancing_action for user-facing display.
For Buy and Sell recommendations, final trade levels must use Risk:Reward between 1:3 and 1:5 only. In numeric fields, risk_reward_ratio means reward divided by risk, so valid values are 3.0 to 5.0. Use price_target as the analyst/fair target. Use take_profit as the trade execution target based on risk/reward. Allowed volatility_level values only: Low, Medium, High, Very High. Allowed rebalancing_action values must follow the final decision and volatility context.
{_horizon_instruction(time_horizon_text)}
