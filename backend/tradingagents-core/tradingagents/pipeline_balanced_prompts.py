...
For Buy and Sell recommendations, the final trade levels must use Risk:Reward exactly 1:3. In numeric fields, risk_reward_ratio means reward divided by risk, so the only valid value is 3.0. Do not use any higher Risk:Reward variant. For Buy, stop_loss must be below entry_price and take_profit must be entry_price + ((entry_price - stop_loss) * 3). For Sell, stop_loss must be above entry_price and take_profit must be entry_price - ((stop_loss - entry_price) * 3). For Buy and Sell, risk_reward_display must be "1:3". Use price_target as an analyst/fair target and take_profit as the trade execution target. If a setup cannot support a valid 1:3 Risk:Reward structure, recommend Hold, Wait for better entry, or Avoid new entry instead of forcing a weak Buy/Sell. Do not invent current_price. Allowed volatility_level values are only Low, Medium, High, or Very High.
{_horizon_instruction(time_horizon_text)}
