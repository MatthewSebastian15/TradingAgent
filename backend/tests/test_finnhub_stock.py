...
def test_get_quote_zero_current_price_unavailable(monkeypatch):
    monkeypatch.setattr("tradingagents.dataflows.finnhub_stock.make_api_request", lambda *a, **k: {"c": 0, "pc": 95})
    try:
        get_quote("AAPL")
...
def test_get_stock_success_returns_csv(monkeypatch):
    monkeypatch.setattr(
        "tradingagents.dataflows.finnhub_stock.make_api_request",
        lambda *a, **k: {"s": "ok", "t": [1779840000], "o": [1], "h": [2], "l": [1], "c": [2], "v": [100]},
    )
...
def test_get_stock_no_data_status_unavailable(monkeypatch):
    monkeypatch.setattr("tradingagents.dataflows.finnhub_stock.make_api_request", lambda *a, **k: {"s": "no_data"})
