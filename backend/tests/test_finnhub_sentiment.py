...
def test_get_news_sentiment_success(monkeypatch):
    monkeypatch.setattr("tradingagents.dataflows.finnhub_sentiment.make_api_request", lambda *a, **k: {"sentiment": {"companyNewsScore": 0.4, "bullishPercent": 0.6}})
...
def test_get_social_sentiment_success(monkeypatch):
    monkeypatch.setattr("tradingagents.dataflows.finnhub_sentiment.make_api_request", lambda *a, **k: {"reddit": [{"mention": 2, "positiveMention": 2}], "twitter": []})
...
def test_social_sentiment_does_not_use_news_as_direct_fallback(monkeypatch):
    monkeypatch.setattr("tradingagents.dataflows.finnhub_sentiment.make_api_request", lambda *a, **k: {"reddit": [], "twitter": []})
    payload = json.loads(get_social_sentiment("AAPL", "2026-05-01", "2026-05-28"))
