...
from tradingagents.dataflows.providers.finnhub_events import (
...
def test_get_earnings_calendar_success(monkeypatch):
    monkeypatch.setattr(
        "tradingagents.dataflows.providers.finnhub_events.make_api_request",
        lambda *a, **k: {"earningsCalendar": [{"date": "2026-05-30"}]},
    )
...
def test_get_stock_earnings_success(monkeypatch):
    monkeypatch.setattr(
        "tradingagents.dataflows.providers.finnhub_events.make_api_request",
        lambda *a, **k: [{"period": "2026-Q1", "surprise": 1}],
    )
