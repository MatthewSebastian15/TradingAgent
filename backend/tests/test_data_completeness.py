...
def test_completeness_groups_empty_do_not_crash():
    ...
    assert report["groups"]["fundamental_data"]["completeness_pct"] == 0
...
def test_completeness_reports_missing_fields():
    ...
    assert report["price_data"]["available_count"] == 2
    assert report["price_data"]["completeness_percent"] == 50.0
    assert report["price_data"]["available_fields"] == ["quote", "historical_price"]
