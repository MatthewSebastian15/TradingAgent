...
def test_idx_uses_100_share_lots():
    sizing = calculate_position_sizing("IDX", entry_price=9500, stop_loss=9000, portfolio_value=7_500_000)
...
def test_us_global_etf_and_fund_do_not_use_lots():
    for market in ("US", "GLOBAL", "ETF", "FUND"):
        sizing = calculate_position_sizing(market, entry_price=100, stop_loss=90, portfolio_value=10_000)
...
def test_crypto_supports_decimal_quantity():
    sizing = calculate_position_sizing("CRYPTO", entry_price=100_000, stop_loss=96_000, portfolio_value=5000)
...
def test_unknown_market_returns_unavailable_reason():
    sizing = calculate_position_sizing("UNKNOWN", entry_price=100, stop_loss=95, portfolio_value=10_000)
...
def test_missing_entry_stop_or_portfolio_returns_unavailable_reason():
    assert calculate_position_sizing("US", None, 95, 10_000).unavailable_reason == "Entry price is required for position sizing."
    assert calculate_position_sizing("US", 100, None, 10_000).unavailable_reason == "Stop loss is required for position sizing."
    assert calculate_position_sizing("US", 100, 95, None).unavailable_reason == "Portfolio value is required for position sizing."
...
def test_risk_per_unit_zero_does_not_crash():
    ...
    assert sizing.unavailable_reason == "Risk per unit is zero because entry price equals stop loss."
...
def test_global_invalid_input_returns_clear_unavailable_reason():
    sizing = calculate_position_sizing("GLOBAL", entry_price=100, stop_loss=100, portfolio_value=10_000)
