...
from tradingagents.dataflows.finnhub_fundamentals import get_basic_financials, get_company_profile, get_financials, get_fundamentals


def test_get_company_profile_success(monkeypatch):
    monkeypatch.setattr("tradingagents.dataflows.finnhub_fundamentals.make_api_request", lambda *a, **k: {"name": "Apple", "finnhubIndustry": "Tech", "marketCapitalization": 1})
...
def test_get_basic_financials_success_maps_selected_metrics(monkeypatch):
    monkeypatch.setattr("tradingagents.dataflows.finnhub_fundamentals.make_api_request", lambda *a, **k: {"metric": {"peTTM": 25, "roeTTM": 20, "beta": 1.1}})
...
def test_get_fundamentals_combines_profile_and_metrics(monkeypatch):
    ...
    monkeypatch.setattr("tradingagents.dataflows.finnhub_fundamentals.make_api_request", fake)
...
def test_get_financials_success_if_enabled(monkeypatch):
    monkeypatch.setattr("tradingagents.dataflows.finnhub_fundamentals.make_api_request", lambda *a, **k: {"financials": [{"year": 2025, "totalAssets": 1}]})
