...
const TEST_DISCLAIMER =
  'This analysis report is provided for informational and educational purposes only. This report is generated by an automated AI-assisted analysis system using third-party provider data and is not a substitute for independent research.';
...
describe('mockReport', () => {
  ...
  it('renders the last full news item in report html', () => {
    ...
    expect(html).not.toContain('Full News GOTO News 11');
    expect(html).not.toContain('Full News List');
  });

  it('renders the mock report disclaimer in mock HTML output', () => {
    const html = buildMockReportHtml({ ...MOCK_HOLD_RESPONSE, disclaimer: TEST_DISCLAIMER });

    expect(html).toContain('Disclaimer');
    expect(html).toContain('informational and educational purposes only');
    expect(html).toContain('third-party provider data');
    expect(html).toContain('not a substitute for independent research');
  ...
  it('renders Key Reasons as a paragraph in mock HTML output', () => {
    ...
    const match = html.match(/<h2>Key Reasons<\/h2>\s*<p class="justified-text">(.*?)<\/p>/s);
  ...
  it('renders Investment Thesis as separated justified paragraphs', () => {
    const html = buildMockReportHtml({
      ...MOCK_RESPONSE,
      investment_thesis: 'First thesis paragraph.\nSecond thesis paragraph.',
    });

    expect(html).toContain('investment-thesis');
    expect(html).toContain(
      '<p class="justified-text thesis-paragraph">First thesis paragraph.</p>'
    );
    expect(html).toContain(
      '<p class="justified-text thesis-paragraph">Second thesis paragraph.</p>'
    );
  });
  ...
  it('renders static company profile in mock HTML output', () => {
    ...
    expect(html).not.toContain('Current Price</th>');
  });

  it('omits Chart & Price summary in mock HTML output', () => {
    const html = buildMockReportHtml(MOCK_RESPONSE);

    expect(html).not.toContain('Chart &amp; Price Summary');
    expect(html).not.toContain('Lookback Days');
    expect(html).not.toContain('Average Volume</th>');
  });

  it('renders shares and ownership table with pie chart in mock HTML output', () => {
    const html = buildMockReportHtml(MOCK_RESPONSE);
  ...
  it('omits Full News List items in mock HTML output', () => {
    const html = buildMockReportHtml(MOCK_RESPONSE);
