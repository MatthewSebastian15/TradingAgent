...
import {
  TEST_HOLD_RESPONSE,
  TEST_IDX_NEWS_UNAVAILABLE_RESPONSE,
  TEST_MISSING_PRICE_RESPONSE,
  TEST_PTRO_WAIT_RESPONSE,
  TEST_RESPONSE,
  TEST_SELL_RESPONSE,
  TEST_TPIA_REDUCE_SCENARIO_RESPONSE,
} from '../../test/analysisResultFixtures';
...
describe('ResultCard risk-engine contract', () => {
  ...
  it('renders the full disclaimer permanently without expand or collapse controls', async () => {
    render(<ResultCard result={TEST_HOLD_RESPONSE} />);
  ...
  it('renders recommendation reasons and risk summary as one paragraph between 100 and 150 words', () => {
    ...
    const result = {
      ...
      analysis_overview: {
        ...TEST_RESPONSE.analysis_overview,
        key_reasons_paragraph: keyReasonsParagraph,
      },
    };
  ...
  it('renders grouped Fundamental filters and only shows the active group metrics', () => {
    const { rerender } = render(<ResultCard result={TEST_RESPONSE} />);
    ...
    rerender(<ResultCard result={{ ...TEST_RESPONSE, peer_comparison: null }} />);
  ...
  it('defaults Fundamental to table and renders only active section charts after Chart is selected', () => {
    const { container } = render(<ResultCard result={TEST_RESPONSE} />);
  ...
  it('renders the News empty state when provider coverage is unavailable', () => {
    render(<ResultCard result={TEST_IDX_NEWS_UNAVAILABLE_RESPONSE} />);
  ...
  it('does not render higher RR variants for valid Buy and Sell results', () => {
    ...
    rerender(<ResultCard result={TEST_SELL_RESPONSE} />);
  ...
  it('does not render removed action-plan fields for Sell result', () => {
    render(<ResultCard result={TEST_SELL_RESPONSE} />);
  ...
  it('renders WAIT wording for no-position scenario', () => {
    render(<ResultCard result={TEST_PTRO_WAIT_RESPONSE} />);
  ...
  it('renders REDUCE wording for existing-position scenario', () => {
    render(<ResultCard result={TEST_TPIA_REDUCE_SCENARIO_RESPONSE} />);
  ...
  it('does not render action plan for invalid actionable trade plan', () => {
    render(<ResultCard result={{ ...TEST_RESPONSE, trade_plan_valid: false }} />);
  ...
  it('handles missing current price without rendering NaN or fake levels', () => {
    render(<ResultCard result={TEST_MISSING_PRICE_RESPONSE} />);
    ...
    render(
      <ResultCard
        result={{
          ...
          price_source: null,
          data_quality: { ...TEST_RESPONSE.data_quality, price_data: 'stale' },
          company_profile: { ...TEST_RESPONSE.company_profile, current_price: 2690 },
          price_chart: {
            ...TEST_RESPONSE.price_chart,
            warning: 'OHLCV_STALE - Latest OHLCV row 2026-05-29 is stale.',
            data_quality: { ...TEST_RESPONSE.price_chart.data_quality, status: 'stale' },
          },
        }}
      />
    );
