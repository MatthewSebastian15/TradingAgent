...
function normalizeAgentId(id = '') {
  const normalized = String(id).trim().toLowerCase().replace(/[^a-z0-9]+/g, '_').replace(/^_+|_+$/g, '');
        ...
        <div className="flex-1 p-3">
          ...
          <div className="flex flex-col gap-1">
            {PIPELINE.map((step) => {
              ...
              return (
                <div key={step.id} className={`flex items-center gap-2.5 px-2 py-1.5 transition-colors duration-200 ${active ? 'bg-bloomberg-surface' : ''}`}>
                  <div className="w-4 h-4 flex items-center justify-center flex-shrink-0">
                    {done ? (
                      <span className="font-mono text-xs text-bloomberg-green">✓</span>
                    ) : active ? (
                      <div className="w-3 h-3 border border-t-transparent rounded-full animate-spin" style={{ borderColor: step.color, borderTopColor: 'transparent' }} />
                    ) : (
                      <span className="font-mono text-xs text-bloomberg-border">○</span>
                    )}
                  </div>

                  <div className="w-0.5 h-4 flex-shrink-0 transition-opacity duration-200" style={{ background: step.color, opacity: done || active ? 1 : 0.2 }} />

                  <span className="font-mono text-xs tracking-wider flex-1 transition-colors duration-200" style={{ color: done ? '#525252' : active ? '#e5e5e5' : '#3d3d3d' }}>
                    {step.label}
                  </span>

                  {done && <span className="font-mono text-xs text-bloomberg-green">DONE</span>}
                  {active && <span className="font-mono text-xs animate-pulse-dot" style={{ color: step.color }}>LIVE</span>}
                </div>
              );
            })}
          </div>
              ...
              <div className="flex flex-col gap-1">
                {log.map((ev) => (
                  <div key={ev.id} className="border-l-2 border-bloomberg-border pl-2 py-0.5">
                    ...
                    {ev.msg && <div className="font-mono text-xs text-bloomberg-muted truncate">{ev.msg}</div>}
                  </div>
                ))}
              </div>
            )}
