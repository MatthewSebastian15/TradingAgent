...
def test_market_prompt_static_prefix_stable(sample_collected_data):
    prompt_a = market_analyst_prompt(
        "BBCA.JK", "2026-06-02", sample_collected_data, "{}", "1 month"
    )
    prompt_b = market_analyst_prompt(
        "BBRI.JK", "2026-06-03", sample_collected_data, "{}", "1 month"
    )
...
def test_initial_analyst_prompts_use_compact_context(sample_collected_data):
    market_prompt = market_analyst_prompt(
        "BBCA.JK", "2026-06-02", sample_collected_data, "{}", "1 month"
    )
    news_prompt = news_social_prompt(
        "BBCA.JK", "2026-06-02", sample_collected_data, "{}", "1 month"
    )
