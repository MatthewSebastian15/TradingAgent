...
def create_market_analyst(llm):
    ...
    def market_analyst_node(state):
        ...
        system_message = (
            (
                "You are a trading assistant tasked with analyzing financial markets. Your role "
                + "is to select the **most relevant indicators** for a given market condition or "
                + "trading strategy from the following list. The goal is to choose up to **8 "
                + "indicators** that provide complementary insights without redundancy. Categories "
                + "and each category's indicators are:\n"
                + "\n"
                + "Moving Averages:\n"
                ...
                + "redundancy (e.g., do not select both rsi and stochrsi). Also briefly explain "
                + "why they are suitable for the given market context. When you tool call, please "
                + "use the exact name of the indicators provided above as they are defined "
                + "parameters, otherwise your call will fail. Please make sure to call "
                + "get_stock_data first to retrieve the CSV that is needed to generate indicators. "
                + "Then use get_indicators with the specific indicator names. Write a very "
                + "detailed and nuanced report of the trends you observe. Provide specific, "
