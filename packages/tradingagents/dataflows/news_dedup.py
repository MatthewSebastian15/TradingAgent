...
def _published_day(article: dict[str, Any]) -> str:
    return str(
        article.get("published_at") or article.get("published_date") or article.get("date") or ""
    )[:10]


def _topic(article: dict[str, Any]) -> str:
    return str(
        article.get("event_type") or article.get("topic") or article.get("bucket") or "general"
    ).lower()
...
def dedup_news_articles_with_metadata(
    ...
    for article in articles or []:
        ...
        for old_title, old_day, old_topic in seen_title_day_topic:
            if (
                _similar(title_key, old_title)
                and (not day or not old_day or day == old_day)
                and topic == old_topic
            ):
    ...
    return result, {
        ...
        "dedup_input_count": len(articles or []),
        "dedup_output_count": len(result),
    }
